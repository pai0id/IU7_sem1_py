...
def three_eight_method(start, end, num, f):  # Метод 3/8
    ...
    while a < end:
        b = a + sect
        delt = (b - a) / 3
        integr += delt * 3 / 8 * (f(a) + 3 * f(a + delt) + f(a) + 3 * f(a + 2 * delt) + f(b))
        a += sect
    return integr
...
integrals = [l_rect_method(st, ed, n1, func), l_rect_method(st, ed, n2, func),  # Вычисление приблиз. знач. интеграла
             three_eight_method(st, ed, n1, func), three_eight_method(st, ed, n2, func)]
tru_integral = antiderivetive(ed) - antiderivetive(st)  # Вычисление интеграла
abs_delts = []
for it in integrals:
    abs_delts.append(it - tru_integral)  # Абсолютные погрешности
relative_delts = []
for it in abs_delts:
...
if i_less_accurate < 2:  # Перебор
    ...
    n = 4
    prev = three_eight_method(st, ed, n, func)
    while n < limit:
        now = three_eight_method(st, ed, n * 2, func)
        if abs(prev - now) < EPS:
            break
        prev = now
        n *= 2
